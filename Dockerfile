...
ENV VIRTUAL_ENV /opt/helpgen-env
ENV PATH $VIRTUAL_ENV/bin:$PATH
RUN python -m venv $VIRTUAL_ENV

# Set the working directory.
WORKDIR /workspaces/helpgen/
...
COPY --chown=user:user poetry.lock* pyproject.toml /workspaces/helpgen/
RUN mkdir -p /home/user/.cache/pypoetry/ && mkdir -p /home/user/.config/pypoetry/ && \
    mkdir -p src/helpgen/ && touch src/helpgen/__init__.py && touch README.md
...
COPY --chown=user:user .pre-commit-config.yaml /workspaces/helpgen/
...
# Expose the app.
ENTRYPOINT ["/opt/helpgen-env/bin/helpgen"]
CMD []
